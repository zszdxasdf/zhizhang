...
//增加账单
  _add() async {
    ...
    await conn
        .prepared('update user set num = num+1 where userId =?', [userId]);
    conn.close();
  }
...
//修改账单
  ...
  void initState() {
    ...
    if (this.widget.action == 0) {
      ...
      var month;
      if (this.widget.bill.billMonth < 10) {
        month = "0${this.widget.bill.billMonth}";
      } else {
        month = "${this.widget.bill.billMonth}";
      }
      var day;
      if (this.widget.bill.billDay < 10) {
        day = "0${this.widget.bill.billDay}";
      } else {
        day = "${this.widget.bill.billDay}";
      }
      var hour;
      ...
      _dateTime = DateTime.parse(
          '${this.widget.bill.billYear}-$month-$day $hour:00:00');
    }
  }
