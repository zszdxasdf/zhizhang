...
class ChartIncomeMonthPage extends StatefulWidget {
  @override
  _ChartIncomeMonthPageState createState() => _ChartIncomeMonthPageState();
}

class _ChartIncomeMonthPageState extends State<ChartIncomeMonthPage> {
  ...
  _getDay() async {
    ...
    Results results = await (await conn.execute(
            'select sum(billAmount) as sum,billDay from bill ' +
                ...
                'And billFromId=0 and userId=$userId group by billDay'))
        .deStream();
    results.forEach((element) {
      //读取日期及其总金额，并存入_daySum
      setState(() {
      ...
      appBar: AppBar(
        ...
        title: SizedBox(
          ...
          child: FlatButton(
              ...
              key: Key('chartincomemonthchoose'),
              onPressed: () {
                _showDatePicker();
              }),
        ),
  ...
  Widget _simpleLine() {
    ...
    var seriesList = [
      new charts.Series<TimeSeriesSales, DateTime>(
        id: 'line',
        colorFn: (_, __) => charts.ColorUtil.fromDartColor(Colors.black), //颜色
    ...
    var seriesList = [
      charts.Series<OrdinalSales, String>(
        ...
        colorFn: (_, __) => charts.ColorUtil.fromDartColor(
            Color.fromARGB(255, 255, 200, 0)), //颜色
