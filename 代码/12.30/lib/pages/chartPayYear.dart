...
class ChartPayYearPage extends StatefulWidget {
  @override
  _ChartPayYearPageState createState() => _ChartPayYearPageState();
}

class _ChartPayYearPageState extends State<ChartPayYearPage> {
  ...
  _getDay() async {
    ...
    Results results = await (await conn.execute(
            'select sum(billAmount) as sum,billMonth from bill ' +
                ...
                'And billFromId=1 and userId=$userId group by billMonth'))
        .deStream();
  ...
  _getTypes() async {
    ...
    results.forEach((element) {
      ...
      setState(() {
        ...
        _num++;
        _height = _num * 60.0; //根据类型数量设置柱形图高度
      });
    });
  ...
  Widget _simplePie() {
    ...
    for (var i = 0; i < 19; i++) {
      if (_typeSum[i] != 0) {
        pie.add(new PieSales(payTypes[i], _typeSum[i] / _sum));
      }
    }
